...
### 3기: DeepSeek Harness + DeepSeek-v4.1-flash + mattpocock-skills (Epic #3 — Tool Calling·Routing)

Epic #2 종료 후 **하네스는 그대로 두고 구동 모델만 교체**해, 하네스×모델 조합을 실측했습니다.

- **모델 교체**: `glm-5.3-flash:cloud` → **`deepseek-v4.1-flash:cloud`**. 목적은 모델 성능 비교가 아니라 **DeepSeek Harness와 모델 간 조합 테스트**였습니다.
- **캐시 히트율 상승**: GLM 사용 시 70~75% 수준이던 캐시 히트율이 DeepSeek 모델로 바꾼 뒤 **99%** 를 기록했습니다. 세션이 길어질수록 이 차이는 그대로 비용·지연·유효 컨텍스트로 이어집니다 — Epic #3은 ADR 작성부터 회귀 검증까지 한 세션에서 진행됐습니다.
- **mattpocock-skills 유지·적극 활용**: ADR 결정 라운드가 상세하고 누락이 거의 없다고 판단해 계속 사용했습니다. Epic #3에서는 `grilling` + `domain-modeling`으로 **4라운드 20개 질문**을 닫아 설계를 확정한 뒤(대안·비목표·미해결 쟁점까지 기록) ADR-0003을 작성하고 구현에 들어갔습니다.

> 여기서 바뀐 것은 **프로젝트를 만드는 에이전트의 구동 모델**입니다. 데모 챗봇의 런타임 LLM은 별개로 `ollama` + `glm-5.3-flash:cloud`를 그대로 사용합니다.

실제 운영에서의 특이점:

- **한 세션에서 Epic 전체 완주**: Epic #3(T1~T7)을 한 세션에서 완료했습니다 — ADR 2건(신규 1·개정 1), 이슈 8장 생성·종료, 커밋 9건, 회귀 16종 222케이스 통과. 2기에서 문제였던 "라운드·컨텍스트 소모"와 "서브에이전트 장기 무응답"이 이번 Epic에서는 재발하지 않았습니다.
- **설계를 먼저 문서로 고정한 뒤 구현**: 인터뷰에서 확정한 결정을 ADR로 먼저 남기고(되돌리기 어려움·맥락 없으면 의아함·실제 트레이드오프 3요건 충족), 그 문서를 기준으로 티켓 7장을 의존 순서로 분해해 구현·검증했습니다. 구현 중 설계와 코드가 어긋나면 문서를 기준으로 되돌렸습니다.
- **자기 결함을 스스로 찾아 고침**: 정량 측정과 실측을 먼저 하도록 해, 아래 결함을 완료 전에 발견·수정했습니다.
  - 마이그레이션 자동 생성이 Prisma가 표현하지 못하는 수동 인덱스(pg_trgm GIN)를 DROP하려 한 문제 → 해당 구문 제거·보존 확인
  - 카테고리 설명의 도메인 전역어가 질의를 흡수한 라우팅 결함(정확도 68.8% → 81.3%)
  - 선언되지 않은 도구를 그대로 실행하던 함수 호출 프로토콜 위반 → 실행 차단·안내 오류 반환
  - 고객 이름만으로는 도구가 `customer_id`를 만들지 못해 되묻던 문제 → 컨텍스트에 고객 UUID 주입
- **서브에이전트 위임 안정화**: 병렬 가능한 T6(프론트 UI)을 백그라운드 서브에이전트에 위임해 **1회 만에 완료**했습니다. 파일 범위를 `frontend/`로 한정하고 요청·응답 계약과 검증 명령을 프롬프트에 고정한 것이 주효했습니다.
- **정량 검증**: 도구 라우팅 임베딩 81.3% / LLM 100%(복합 의도 top-2 병합 recall 87.5% / 100%), 상거래 시드 멱등성(재실행 후 건수 동일·중복 0)·시나리오 실측, 실 엔드투엔드 A/B 두 모드 실제 배송지연 답변, 회귀 16종 222케이스 — 완료 판정을 수치로 남김.
...
> 3기(모델 교체)에서는 위 두 항목이 실질적 병목으로 나타나지 않았습니다 — Epic #3을 한 세션에서 완주했습니다. 다만 이는 체감상의 개선이며, 하네스의 대기·pause 동작 자체가 바뀌었다는 뜻은 아닙니다.
